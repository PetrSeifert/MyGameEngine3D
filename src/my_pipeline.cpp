...
MyPipeline::~MyPipeline(){
    ...
    vkDestroyPipeline(myDevice.device(), graphicsPipeline, nullptr);
}
...
std::vector<char> MyPipeline::readFile(const std::string& filePath){
    ...
    if (!file.is_open()){
        std::cout << ENGINE_DIR << " " << filePath << std::endl;
...
void MyPipeline::defaultPipelineConfigInfo(PipelineConfigInfo& configInfo){
    configInfo.inputAssemblyInfo.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO;
    ...
    configInfo.bindingDescriptions = MyModel::Vertex::getBindingDescriptions();
    configInfo.attributeDescriptions = MyModel::Vertex::getAttributeDescriptions();
};