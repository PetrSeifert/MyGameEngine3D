...
MyDevice::MyDevice(MyWindow& window) : window{ window } 
{
	createInstance();
	setupDebugMessenger();
	createSurface();
...
void MyDevice::createSurface() { window.createWindowSurface(instance, &surface_); }

bool MyDevice::isDeviceSuitable(VkPhysicalDevice device) 
{
...
void MyDevice::populateDebugMessengerCreateInfo(
...
void MyDevice::hasGflwRequiredInstanceExtensions() 
{
...
void MyDevice::copyBufferToImage(
