...
template <>
struct std::hash<MyModel::Vertex> {
...
MyModel::MyModel(MyDevice &device, const MyModel::Builder &builder) : myDevice{device}{
...
std::unique_ptr<MyModel> MyModel::createModelFromFile(MyDevice& device, const std::string& filepath){
    Builder builder{};
...
void MyModel::createVertexBuffers(const std::vector<Vertex> &vertices){
    ...
    vertexBuffer = std::make_unique<MyBuffer>(
        myDevice, 
        vertexSize, 
        vertexCount, 
...
void MyModel::createIndexBuffers(const std::vector<uint32_t>& indices){
    ...
    indexBuffer = std::make_unique<MyBuffer>(
        myDevice,
        indexSize,
        indexCount,
    ...
    myDevice.copyBuffer(stagingBuffer.getBuffer(), indexBuffer->getBuffer(), bufferSize);
}
